...
public extension Abort {
    func hx() -> HXError {
		let using = { @Sendable (req: Request, abort: Self, htmx: Bool) async throws in
    ...
    func hx(_ using: @Sendable @escaping (_ req: Request, _ abort: Abort, _ htmx: Bool) async throws -> Response) -> HXError {
